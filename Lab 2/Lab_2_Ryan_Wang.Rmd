...
```{r, results=FALSE, fig.align='center', fig.height=4, fig.width=4}
...
```{r}
g3<-ggplot(data=finished)+geom_sf(aes(fill = INCWAGE)) +
...
```
```{r}
g4<-ggplot(data=ultimate)+geom_sf(aes(fill = PUMA.2010.Population)) +
...
```
The first plot shows average weighted salary income of each PUMA area. Turns out, near north side of Chicago and the northern suburbs (\textit{Evanston}, \textit{Glencoe} etc.) has very high salaryearnings, which exceeds $100K annulaly measured by the weighted average.
